...
export type NewThreadChunk = {
  type: "new-thread";
  threadId: string;
};
export type NewThreadTitleChunk = {
  type: "new-thread-title";
  threadId: string;
  title: string;
};
...
export type CreateThreadChunk =
  | NewThreadChunk
  | NewThreadTitleChunk
  | AssistantMessageChunk;

const CHAT_SYSTEM_PROMPT = `You are Lumen, a friendly Christian AI assistant who helps users grow in their faith and understand Scripture. Think of yourself as a knowledgeable pastor who's having a casual conversation over coffee.
...
const CHAT_WEB_SEARCH_SYSTEM_PROMPT = `You are Lumen, a Christian AI assistant dedicated to helping users grow in their faith and understand Scripture. Your primary focus is discussing the Bible, Christian theology, and matters of faith.
...
const CHAT_TITLE_SYSTEM_PROMPT = `You are a title generator for Lumen, an AI assistant that helps users understand and explore the Bible. Your task is to create a concise, descriptive title for a chat conversation based on the user's initial message.
...
async function* createThread(
  ...
  options?: SendMessageOptions & { isWebSearchEnabled?: boolean },
): AsyncGenerator<CreateThreadChunk> {
  if (!ctx.user?.id) {
    throw new Error("User not found");
  }

  const thread = await ctx.db.chatThread.create({
    data: { user_id: ctx.user.id, title: "New Chat" },
  });
  ...
  const userMessagePromise = ctx.db.chatMessage.create({
    data: {
      role: "USER",
  ...
  const title = titleResponse.choices[0]?.message.content ?? "Untitled Chat";
  const updateThreadTitlePromise = ctx.db.chatThread.update({
    where: { id: thread.id },
    data: { title },
  });
...
async function* sendMessageWithWebSearch(
  ...
  const messages: ResponseInputItem[] = [
    {
      role: "system",
      content: CHAT_WEB_SEARCH_SYSTEM_PROMPT,
    },
    ...chatMessages,
  ];
